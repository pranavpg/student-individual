									  ...
									  <?php for ($k = 1;$k <= $columnCount;$k++) { ?>
											<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea" >
												<?php if ( isset($firstColumns[$j]) && $k == 1 && isset($firstColumns) && !empty($firstColumns) && !empty(trim($firstColumns[$j]))  && $firstColumns[$j]!=""  ) { 
														$newClass = $firstColumns[$j]!=" -1 "?"newcheck":"";
													?>
													<span class="textarea form-control form-control-textarea {{$firstColumns[$j]}}" ></span>
													<div style="display:none">
												...
												<?php }else { 
																    ...
																    if ($answerExists) {
																		if(isset($practise['user_answer'][0][0][$newJ]['col_' . $k])){
																			// $disaplyAns =  $practise['user_answer'][0][0][$newJ]['col_' . $k]=="-1"?"":$practise['user_answer'][0][0][$newJ]['col_' . $k];
																			$disaplyAns =  $practise['user_answer'][0][0][$newJ]['col_' . $k];
																		}
																		else if(!empty($practise['user_answer'][0]['text_ans'][0][$newJ]['col_' . $k])){
																				$disaplyAns =  $practise['user_answer'][0]['text_ans'][0][$newJ]['col_' . $k];
																		}
												            		}
																?>

															<span class="textarea form-control form-control-textarea  {{$disaplyAns}}" role="textbox" disabled contenteditable placeholder="Write here..." style="">{!!$disaplyAns!!}</span>
															<div style="display:none">
																<textarea name="col[]">
																	<?php
															            if ($answerExists) {
...
<script type="text/javascript">
	$('.-1').each(function(){
		...
		if(value == ""){
			$(this).css("background-color","#007480")
