<style>
  .paragraph-noun .list-inline-item{
    display: flex;
  
  }
  .paragraph-noun .list-inline-item .form-control{
    min-width: 20px;
    width: 30px;
  }
</style>
          ...
          if(str_contains($value,'@@')){
            $outValue = preg_replace_callback('/@@/',
                ...
                //  $str = '<span class="textarea d-inline-flex mw-20 form-control form-control-textarea conversation_answer_'.$count.'" role="textbox" contenteditable placeholder="Write here...">'.$ans.'</span><input type="hidden" name="text_ans['.$key.']['.$count.']" value="'.$ans.'">';
                  $str = '<span class="ml-1 resizing-input" style="letter-spacing: 1px;"><input type="text"  name="text_ans[]" class="write_in_blank form-control form-control-inline" value="'.$ans.'"><span style="display:none"></span></span>';
                  return $str;
                }
                , $value);
          } else
      ...
      ?>
        <div class="paragraph-noun_box">
            <ul class="list-inline">
                <li class="list-inline-item" style="letter-spacing: 1px;">
                  {!! $outValue !!}
                </li>
            </ul>
        </div>
...
$(document).on('keypress', '.write_in_blank', function() {
    // $(this).css('min-width','7px');
    $(this).css('border-bottom','2px solid #ced4da')
