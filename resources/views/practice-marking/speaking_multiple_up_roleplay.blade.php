...
<script> 
  $(function () {
    $('#abc-{{$practise["id"]}}').find(".selected_option").click(function () {
      var content_key = $(this).attr('data-key');
   
      $("#abc-{{$practise['id']}}").find('.selected_option').not(this).toggleClass('d-none');
      $('#abc-{{$practise["id"]}}').find('.selected_option_description_'+content_key).toggleClass('d-none');
      $('#abc-{{$practise["id"]}}').find('.selected_option_description_'+content_key).show();
      $(this).toggleClass('btn-bg');
      roleplayDependent(content_key)
      
      if( $('#abc-{{$practise["id"]}}').find('.selected_option_description:visible').length>0 ){
        $(".showPreviousPractice_{{$practise['id']}}").show();
        $('#abc-{{$practise["id"]}}').find('.is_roleplay_submit').val(0);
      }else{
        $('#abc-{{$practise["id"]}}').find('.is_roleplay_submit').val(1);
