...
if($practise['typeofdependingpractice'] =="get_answers_generate_quetions") {
															 	...
															 	if(substr_count($question,"@@")>2){
																	echo $outValue = preg_replace_callback('/@@/', function ($m) use (&$key1, &$newAns, &$data, &$question, &$k, &$ansData, &$newFlag){
																		...
																		$str ='<span class="resizing-input1">
																 				<span contenteditable="true" class="enter_disable spandata fillblanks stringProper" style="min-width:'.$style.' !important;">'.$ans.'</span>
		...
		?>
			<div class="reading-no-blanks_form" id="reading-no-blanks_form-<?php echo $practise['id'];?>">
			    <input type="hidden" class="topic_id" name="topic_id" value="{{$topicId}}">
			    <input type="hidden" class="task_id" name="task_id" value="{{$taskId}}">
			    <input type="hidden" class="is_save" name="is_save" value="">
			    <input type="hidden" class="practise_id" name="practise_id" value="{{$practise['id']}}">
					...
					for(var e=0;e<2;e++) {
						$.each(ans[e], function (key,value) {
								...
								$('.appenddata_'+e).find('#'+key).html('<span class="resizing-input1">\
				 				<span contenteditable="true" class="enter_disable spandata fillblanks stringProper"  style="min-width:'+size+'" >'+value+'</span>\
