...
if(empty($practise['dependingpractiseid']) &&  empty($practise['is_dependent']) ){
			...
			foreach($allOptions as $allOption){ ?>
				<div class="suggestion-list d-flex flex-wrap mb-4 justify-content-center">
					<?php foreach($allOption as $option){?>
						<div class="d-inline-flex flex-grow-1 suggestion_box justify-content-center w-25 py-0">
							<?php echo $option;?>
						</div>
					<?php }?>
				</div>
		...
		if($practise['type'] == "two_blank_table_up_writing_at_end_up"){
			...
			foreach($exploded_question as $counttt=>$question){
				...
				if(count($displayQuestion) > 1){ ?>
					<div class="form-group d-flex align-items-start form-group-label mb-4">
						<span class="label"><?php echo $displayQuestion[0];?></span>
						<span class="textarea form-control form-control-textarea two_blank_table_up_writing_at_end_up_textarea" role="textbox" disabled contenteditable="" placeholder="Write here..."><?php echo $val;?></span>
						<div style="display:none">
					<textarea name="blanks_up[]"><?php echo $val;?></textarea>
					</div>
					</div>
	...
	<?php
        ...
        // add isset for remove black screen error
    		...
    		<?php
				...
				if($practise['type'] == "writing_at_end_four_blanks_table" || $practise['type'] == "writing_at_end_up_four_blanks_table"){
					...
					foreach($exploded_question as $counttt=>$question){
						...
						<div class="form-group d-flex align-items-start form-group-label mb-4">
							<span class="label"><?php echo $displayQuestion[0];?></span>
							<span class="textarea form-control form-control-textarea blank_textarea stringProper" role="textbox" disabled contenteditable="" placeholder="Write here...">{{isset($tempPracticeData['user_answer'][1][$counttt]) ? $tempPracticeData['user_answer'][1][$counttt] :''}}</span>
							<div style="display:none">
						<textarea name="blanks_up[]"><?php echo $val;?></textarea>
						</div>
						</div>
		...
		if(data["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){
			...
			if(data["{{$practise['id']}}"]["dependant_practise_id"] !=""  ){
				 ...
				 setTimeout(function(){ 
					...
					$(document).find(".showPreviousPractice_"+data["{{$practise['id']}}"]["dependant_practise_id"]).find('textarea').attr("name","test");
