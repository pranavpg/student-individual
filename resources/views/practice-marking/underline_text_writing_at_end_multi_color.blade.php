						...
						@if(!empty(trim($value)))
							<p class="underline_text_list_item q{{$j}}" data-qno="{{$key}}">
								...
								<div class="form-group focus">
									<span class="textarea form-control form-control-textarea stringProper enter_disable" role="textbox" disabled contenteditable placeholder="Write here...">{{($answerExists && !empty($answers[$j])?$answers[$j]:"")}}</span>
