...
$(document).ready(function(){
	...
	$('.underline_text_list_item').each(function(key){
		var paragraph="";
		var str = $(this).text();
		var $this =$(this);
		str.replace(/[ ]{2,}/gi," ");
	...
	if( answers !=""){
		...
		if( parsedAnswer!==undefined && parsedAnswer!==null ){
			...
			$.each(parsedAnswer, function(key, value) {
				 $('.underline_text_list_item').find('#'+value.i).addClass('bg-success');
				 $('.underline_text_list_item').find('#'+value.i).find('input').removeAttr('disabled');
			});
		}
	}
