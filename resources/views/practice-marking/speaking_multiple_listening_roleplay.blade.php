...
@if(!empty($practise['is_dependent']) && $practise['is_dependent']==1 && $data[$practise['id']]['isRolePlayPractice']==1 && $practise['typeofdependingpractice']!='get_answers_generate_quetions' )
  ...
  var data115=[]
    function roleplayDependent(content_key){
      data115["{{$practise['id']}}"]["typeofdependingpractice"] = "{{$data[$practise['id']]['typeofdependingpractice']}}";
      data115["{{$practise['id']}}"]["dependant_practise_question_type"]  = "{{$data[$practise['id']]['dependant_practise_question_type']}}";
      if(data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_hide_show" || data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_parentextra" || data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_gen_que_double" || data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero_parentextra_get_questions_and_answers" || data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero" || data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_get_ans" || data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view"){
        data115["{{$practise['id']}}"]["dependant_practise_task_id"] =  "{{$data[$practise['id']]['dependant_practise_task_id']}}";
        data115["{{$practise['id']}}"]["dependant_practise_id"] = "{{$data[$practise['id']]['setFullViewFromPreviousPractice']}}";
        $(function () {
          $('.cover-spin').fadeIn();
        });
        if(data115["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){
          // IF DEPENDENT PRACTICE BELONGS TO SAME TASK THEN USE BELOW CODE.
          if(data115["{{$practise['id']}}"]["dependant_practise_id"] !="" ){
            setTimeout(function(){
              //-----for level 0-------------
              ...
              if(data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero" ){
                
                if( data115["{{$practise['id']}}"]["dependant_practise_question_type"] == "writing_at_end_speaking_up" || data115["{{$practise['id']}}"]["dependant_practise_question_type"] == "single_speaking_up_writing") {
          ...
          var baseUrl = "{{url('/')}}";
          data115["{{$practise['id']}}"]["dependant_practise_topic_id"] = "{{$data[$practise['id']]['dependant_practise_topic_id']}}";
          var dependentURL = baseUrl+'/topic/'+data115["{{$practise['id']}}"]["dependant_practise_topic_id"]+'/'+data115["{{$practise['id']}}"]["dependant_practise_task_id"]+'?n='+data115["{{$practise['id']}}"]["dependant_practise_id"]+'&user_id={{ isset($studentId)?$studentId:'' }}';
          $.get(dependentURL,  //
          function (dataHtml, textStatus, jqXHR) {  // success callback
            var prevHTML = $(dataHtml).find('.course-tab-content').find('#abc-'+data115["{{$practise['id']}}"]["dependant_practise_id"]).find('form').find('.selected_option_description_'+content_key).html();        
            // alert(data115["{{$practise['id']}}"]["dependant_practise_id"]);
            $('.showPreviousPractice_'+data115["{{$practise['id']}}"]["dependant_practise_id"]).html(prevHTML);
            $(".showPreviousPractice_"+data115["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
            $(".showPreviousPractice_"+data115["{{$practise['id']}}"]["dependant_practise_id"]).find('ul.list-buttons, input:hidden, textarea').remove();
            if(data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view" && data115["{{$practise['id']}}"]["dependant_practise_question_type"] == "speaking_writing"){
                $(".showPreviousPractice_"+data115["{{$practise['id']}}"]["dependant_practise_id"]).find('fieldset').remove();
            }
            if(data115["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero"){
              
              if(data115["{{$practise['id']}}"]["dependant_practise_question_type"] == "writing_at_end_speaking_up" || data115["{{$practise['id']}}"]["dependant_practise_question_type"] == "speaking_writing") {
                $(".showPreviousPractice_"+data115["{{$practise['id']}}"]["dependant_practise_id"]).find('fieldset').remove();
              }
            }
            $('.cover-spin').fadeOut();
