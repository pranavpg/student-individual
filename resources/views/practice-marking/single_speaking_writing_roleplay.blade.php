...
<form class="form_{{$practise['id']}}">
	...
	<?php  //dd($practise); //echo '<pre>'; print_r($practise); echo '</pre>'; ?>
	<div class="component-two-click mb-4">
		...
		<div class="two-click d-flex flex-wrap justify-content-center mb-4 w-100">
			...
			?>
			<a href="#!" class="btn btn-dark selected_option selected_option_{{$key}} mb-2 mr-3" data-ans_count={{$answer_count}} data-key="{{$key}}" data-show_dependent_error="{{ ( !empty($practise['dependingpractiseid']) && empty($practise['dependingpractise_answer'][$answer_count]) )?1:0 }}" >{{$value}}</a>
			...
			<div class="content-box multiple-choice d-none selected_option_description selected_option_description_{{$k}}">
				...
				<div class="dependant_pr_{{$practise['id']}}"  style="margin-top:15px; border: 2px dashed gray; border-radius: 13px; ">
					...
					<div style="display:{{$answer_block}}">
						...
						<div class="two-click d-flex flex-wrap justify-content-center mb-4 w-100">
							<a href="javascript:void(0);" data-toggle="modal" data-target="#read_text_modal_{{$answer_count}}" class="btn btn-dark selected_option_hide_show">{{ !empty($read_text_tab) ? $read_text_tab : "Read Text" }}</a>
						</div>

						<div class="modal" id="read_text_modal_{{$answer_count}}" tabindex="-1" role="dialog" aria-labelledby="deoModalTitle" aria-hidden="true">
		...
		<!-- <ul class="list-inline list-buttons">
			<li class="list-inline-item"><button type="button" class="btn btn-secondary saveBtn submitBtn_{{$practise['id']}}"   data-pid="{{$practise['id']}}"
				data-toggle="modal" data-is_save="0" data-target="#exitmodal">Save</button></li>
				<li class="list-inline-item"><button type="button"
					class="btn btn-secondary submitBtn submitBtn_{{$practise['id']}}" data-pid="{{$practise['id']}}"   data-is_save="1">Submit</button></li>
				</ul> -->
			</form>
				...
				$(function() {
					...
					$(".form_{{$practise['id']}}").find(".selected_option").click(function() {
						...
						if ($(".form_{{$practise['id']}}").find('.selected_option_description:visible').length > 0) {
							...
							$(".form_{{$practise['id']}}").find('.submitBtn').attr('data-is_save', 0);
						} else {
          ...
          function (dataHTML, textStatus, jqXHR) {  // success callback
          	...
          	var prevHTML = $(dataHTML).find('.course-tab-content').find('#abc-'+data["{{$practise['id']}}"]["dependant_practise_id"]).find('.selected_option_description_'+content_key).html();
          	...
          	if(data["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero"){
          		...
          		if(data["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero_parentextra_get_questions_and_answers" ||data["{{$practise['id']}}"]["typeofdependingpractice"]== "set_full_view_remove_zero"){
          			if( data["{{$practise['id']}}"]["dependant_practise_question_type"] == "three_blank_table_speaking_up" ||  data["{{$practise['id']}}"]["dependant_practise_question_type"] == "writing_at_end_speaking_up" ||  data["{{$practise['id']}}"]["dependant_practise_question_type"] == "single_speaking_up_writing") {
          				$(".showPreviousPractice_{{$practise['id']}}").find('fieldset').remove();
          			}
          		}
