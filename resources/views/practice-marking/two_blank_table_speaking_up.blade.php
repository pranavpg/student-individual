...
<?php
	...
	if(empty($practise['dependingpractiseid']) &&  empty($practise['is_dependent']) ){
		...
		if(isset($practise['is_roleplay']) && $practise['is_roleplay'] == 1){ ?>
			@include('practice.blank_table_speaking_up_roleplay') <?php 
		} else {
			...
			if(str_contains($practise['question'],'#%')  ){
				...
				if(!empty($exploded_question_by_slasht[1])) {
					$roleCards = 0;
						...
						if(count($check_custom_table_header) > 0){ ?>
							<div class="dark-buttons row mb-4">
								...
								?>
									...
									<div class="modal" id="deoModal_<?php echo $practise['id'];?>_<?php echo $coun;?>" tabindex="-1" role="dialog" aria-labelledby="deoModalTitle" aria-hidden="true" style="z-index:100000000;">
						...
						if(count($check_custom_table_header) > 0){ ?>
							<div class="dark-buttons row mb-4">
								...
								foreach($check_custom_table_header as $coun=>$custom_option){
									$custom_option = explode("#%",$custom_option);
								?>
									<div class="col-4 mb-4"><a href="javascript:void(0);" data-toggle="modal" data-target="#deoModal_<?php echo $practise['id'];?>_<?php echo $coun;?>" class="btn btn-dark" style="display:block; text-align:center;z-index: 1500000;"><?php echo $custom_option[0];?></a></div>
									<div class="modal" id="deoModal_<?php echo $practise['id'];?>_<?php echo $coun;?>" tabindex="-1" role="dialog" aria-labelledby="deoModalTitle" aria-hidden="true" style="z-index: 1500000;">
										<div class="modal-dialog modal-dialog-centered deoModal" role="document">
											<div class="modal-content">
											<div class="d-flex justify-content-end w-100"><button type="button" class="btn btn-secondary  btn-sm mb-2" data-dismiss="modal">&times;</button></div>
												<div class="modal-header">
 		...
 		?>
			...
			<div class="table-container">
		    	...
		    	<input type="hidden" class="deleted_audio_{{$practise['id']}}_0" name="two_blank_table_speaking_up" value="0">
				...
				<?php
					...
					if( str_contains($practise['question'],'#%') && $roleCards ==0 ){
						$check_custom_table_header=array();
						$exploded_question = explode('#%', $practise['question']);
						$read_text_tab = $exploded_question[0];
					...
					$countElement = 0; 
				?>
				@if(!empty($read_text_description ) && $roleCards ==0 )
				<div class="two-click d-flex flex-wrap justify-content-center mb-4 w-100">
					<a href="javascript:void(0);" data-toggle="modal" data-target="#read_text_modal_{{$practise['id']}}" class="btn btn-dark mb-3 selected_option_hide_show">{{ !empty($read_text_tab) ? $read_text_tab : "Read Text" }}</a>
				</div>

				<div class="modal" id="read_text_modal_{{$practise['id']}}" tabindex="-1" role="dialog" aria-labelledby="deoModalTitle" aria-hidden="true">
					<div class="modal-dialog modal-dialog-centered deoModal" role="document">
						<div class="modal-content">
						<div class="d-flex justify-content-end w-100"><button type="button" class="btn btn-secondary  btn-sm mb-2" data-dismiss="modal">&times;</button></div>
							<div class="modal-header">
								<h5 class="modal-title text-center" id="deoModalTitle">{!! !empty($read_text_tab) ? $read_text_tab : "Read Text" !!}</h5>
							</div>
						...
						if(isset($totalColumnCounts)){
							...
							<?php for ($k = 1;$k <= $columnCount;$k++) { ?>
									<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea">
										...
										<?php }elseif (isset($secondColumns[$j]) && $k == 2 && isset($secondColumns) && !empty($secondColumns) && !empty(trim($secondColumns[$j]))) { $rowflag=0;  ?>
				...
				<?php }?>
					...
					<!-- <ul class="list-inline list-buttons">
						<li class="list-inline-item">
							<button type="button" class="btn btn-secondary blankTableBtn_{{$practise['id']}}" data-is_save="0" >Save</button>
						</li>
						<li class="list-inline-item">
							<button type="button" class="btn btn-secondary blankTableBtn_{{$practise['id']}}" data-is_save="1"  >Submit</button>
						</li>
					</ul> -->
		...
		<script type="text/javascript">
			...
			$(document).on('click','.blankTableBtn_{{$practise["id"]}}' ,function() {
				var reviewPopup = '{!!isset($reviewPopup)?$reviewPopup:''!!}';
			var markingmethod = '<?php echo isset($practise["markingmethod"])?$practise["markingmethod"]:""; ?>';
					if(markingmethod =="student_self_marking"){
						if($(this).attr('data-is_save') == '1'){
							var AllAudio = new Array();
		                    var checkAudioAva = new Array();
							$('#blank_table_form_{{$practise['id']}} .main-audio-record-div').each(function(){
							...
							var black= $(".dark-buttons").html();
							
							$("#selfMarking_{{isset($practise['id'])?$practise['id']:''}}").find('#practise_div').prepend("<div style='display:flex;flex-wrap: wrap;'>"+black+"</div>");
		...
		<?php } ?>
			...
			<div class="table-container mb-4 text-center">
				<div class="table m-auto">
							...
							foreach( $practise['dependingpractise_answer'][0][0] as $key => $value){
								if( !empty($practise['dependingpractise_answer'][0][1][$key] )){
									...
									foreach($practise['dependingpractise_answer'][0][1][$key] as $k=>$v){
										if( !in_array($value['col_'.$m].' ', $table_header) ){
					...
					} elseif( !empty($practise['dependingpractise_answer']) && !empty($practise['typeofdependingpractice']) && $practise['typeofdependingpractice']=='get_answers_put_into_quetions'  ) {
						foreach( $practise['dependingpractise_answer'][0][0] as $key => $value){
						  ...
						  <?php for ($k = 1;$k <= $columnCount;$k++) { ?>
								<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea">
									...
									<?php }elseif (isset($secondColumns[$j]) && $k == 2 && isset($secondColumns) && !empty($secondColumns) && !empty(trim($secondColumns[$j]))) {  ?>
										<span class="textarea form-control form-control-textarea" {{ ($data[$practise['id']]['isSecondColContentEditable']==1)?"contenteditable":""}}><?php echo $secondColumns[$j]; ?></span>
										<div style="display:none">
											<textarea name="col[]"><?php echo $secondColumns[$j]; ?></textarea>
											<input type="hidden" name="true_false[]" value="false" />
										</div>
		...
		if(data["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){
			if(data["{{$practise['id']}}"]["dependant_practise_id"] !=""  ){
				 setTimeout(function(){ 
					 ...
					 if(data["{{$practise['id']}}"]["typeofdependingpractice"] != "get_questions_and_answers"){
