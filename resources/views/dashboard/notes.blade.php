...
<main class="dashboard">
	<div class="container-fluid">
		<div class="row">
	        ...
	        @endif
	        <h1 class="pageheading m-auto"><span><i class="fas fa-file-alt" aria-hidden="true"></i></span> Notes</h1>
	     </div>
					...
					<div class="row">
						<div class="col-12 col-sm-7 col-md-6 col-lg-8 col-xl-8 d-none d-sm-block d-xl-none notes-selection">
							<select class="col-md-6 custom-select2-dropdown-nosearch" id="onchange_small" style="white-space: nowrap;text-overflow: ellipsis;overflow: hidden;min-width: 300px;">
								...
								@endphp
								<option value="<?php echo $course['level_id'];?>" data-new-id1="{{$key}}" ><?php echo $course['course']['coursetitle'];?> - <?php echo $course['level']['leveltitle'];?></option>
								@endforeach
							</select>
						</div>
...
<div class="modal fade" id="description-model" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">
					<i class="fas fa-file-alt"></i> <span id="coursetitleview">GES</span> - NOTE
				</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">×</span>
				</button>
...
<script type="text/javascript">
        ...
        $('.openmodel').click(function(){
	    	...
	    	// alert(courseName);
	    	$('#coursetitleview').text(courseName);
...
<script>
	...
	$(document).on('change',"#onchange_small",function(){
		let open_button_id = $(this).find(':selected').attr('data-new-id1');

		window.data_ids = open_button_id;
		var current_val = $("#onchange_small").val();
		var download_course_id = $(this).find(':selected').attr('data-id');
		$('#pdf_course_id').val(download_course_id);
	 	$('#pdf_level_id').val(current_val);
	  	var get_rec     = $("#onchange_large option[value='"+current_val+"']" ).text();
...
$(document).on('click',"#pills-tab",function(){
	topicdata();
	let value = $("#pills-tab").find('.checkparent.active').attr("data-2-id");
	$(".tab-pane.fade").removeClass('active show');
	$("#pills-"+value).addClass('active show');
	var get_rec     = $("#onchange_filter option[value='"+value+"']" ).text();
	$("#pills-tab option[value='"+get_rec+"']" ).prop('selected', true);
	$("#select2-onchange_filter-container").text(get_rec);
	let index = $("#pills-tab li.nav-item .nav-link.active").attr("data-id");
	$('#pdf_course_id').val(index);
	var index_leavl = $("#pills-tab li.nav-item .nav-link.active").attr("data-2-id");
	$('#pdf_level_id').val(index_leavl);
	topicfilter();
});
</script>
