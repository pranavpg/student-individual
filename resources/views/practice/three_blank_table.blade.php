...
<?php
		...
		$exploded_question = explode(PHP_EOL,isset($practise['question'])?$practise['question']:'');
	}
    	...
    	<div class="table-container">
    		...
    		<?php
				if($practise['type'] == "two_blank_table_up_writing_at_end_up" || $practise['type'] == "writing_at_end_four_blanks_table" || $practise['type'] == "writing_at_end_up_four_blanks_table"){
					$wholeQuestion = explode("/t",isset($practise['question'])?$practise['question']:'');
				...
				elseif ($practise['type'] == "three_blank_table" || $practise['type'] == "three_blank_table_listening" || $practise['type'] == "three_blanks_table_listening" || $practise['type'] == "three_table_option" || $practise['type'] == "three_blank_table_speaking" || $practise['type'] == "three_table_option_speaking" || $practise['type'] == "three_blank_table_speaking_up" || $practise['type'] == "three_table_option_listening")
				{
				    $columnCount = 3;
					$columnClass = 'w-33';
					$tableHeaderColumnClass = (count($table_header)==1)?'w-100':'w-33';
				}
				...
				<div class="table m-auto">
					...
					<div class="table-heading thead-dark d-flex justify-content-between">
						...
						<div class="d-flex justify-content-center align-items-center th <?php echo $tableHeaderColumnClass; ?>"><?php echo $table_head; ?>
							<div style="display:none;">
								<textarea name="col[]"><?php echo !empty($table_head)?$table_head:" &nbsp;"; ?></textarea>
					...
					<?php }
						if(!empty($table_header) && count($table_header) < $columnCount){ 
							$addExtraCol = $columnCount-count($table_header);
							for($c=0;$c<$addExtraCol;$c++){
					?>
						
						<div style="display:none">
							<textarea name="col[]">^D^</textarea>
							<input type="hidden" name="true_false[]" value="false" />
						</div>
					...
					if(isset($firstColumns) && !empty($firstColumns)){
						...
						$fcData = array();
						$scData = array();
						$data[$practise['id']]['isSecondColContentEditable']= 0;
						if( !empty($practise['dependingpractise_answer']) && !empty($practise['typeofdependingpractice']) && $practise['typeofdependingpractice']=='get_questions_and_answers'  ) {
							
							if(!empty($practise['user_answer'])){
								foreach( $practise['user_answer'][0]['text_ans'][0] as $key => $value){
									if($key>0){
										array_push($fcData,$value['col_1']);
										array_push($scData,$value['col_2']); 
									
										$firstColumns =$fcData;
										$secondColumns =$scData;
										$totalColumnCounts = count($firstColumns);
										$data[$practise['id']]['isSecondColContentEditable']= 1;
									}
								}
							} else {
						...
						<div class="table-row thead-dark d-flex justify-content-between">
						  <?php for ($k = 1;$k <= $columnCount;$k++) { ?>
								<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea">
									...
									<?php }elseif ($k == 3 && isset($thirdColumns) && !empty($thirdColumns) && !empty(trim($thirdColumns[$j]))) { $rowflag=0; ?>
									...
									<?php }elseif ($k == 6 && isset($sixColumns) && !empty($sixColumns) && !empty(trim($sixColumns[$j]))) {  $rowflag=0; ?>
			...
			<?php }?>
				...
				<ul class="list-inline list-buttons">
					<li class="list-inline-item">
						<button type="button" class="btn btn-primary blankTableBtn_{{$practise['id']}}" data-pid="{{$practise['id']}}" data-is_save="0" >Save</button>
					</li>
					<li class="list-inline-item">
						<button type="button" class="btn btn-primary submitBtn blankTableBtn_{{$practise['id']}}" data-pid="{{$practise['id']}}" data-is_save="1">Submit</button>
					</li>
				</ul>
    ...
    var markingmethod = '<?php echo isset($practise["markingmethod"])?$practise["markingmethod"]:""; ?>';
            if(markingmethod =="student_self_marking"){
                ...
                if($(this).attr('data-is_save') == '1'){
					...
					$("#selfMarking_{{isset($practise['id'])?$practise['id']:''}}").find('.textarea').css("word-break","break-all");
                }
            }
...
?>
		<div class="table-container">
    		...
    		<?php
				...
				if( !empty($practise['dependingpractise_answer']) && !empty($practise['typeofdependingpractice']) && $practise['typeofdependingpractice']=='get_answers_put_into_quetions'  ) {
					if( $practise['depending_practise_details']['question_type']=='match_answer' ){
						$explode_question_dep = explode('#@',$practise['depending_practise_details']['question'][0]);
						$table_header = explode('@@', $explode_question_dep[0]);
						array_push($table_header,'Full Sentence');
					}
					else if( $practise['depending_practise_details']['question_type']=='two_blank_table_speaking_up' ){
						$explode_question_dep = explode( PHP_EOL, $practise['depending_practise_details']['question']);
						$table_header = explode('@@', $explode_question_dep[0]);
						array_push($table_header,'Translation');
					}
				}
				//pr($table_header);
				$answerExists = false;
			...
			<div class="table-container mb-4 text-center">
				<div class="table m-auto">
					...
					<div class="table-heading thead-dark d-flex justify-content-between">
						<?php foreach ($table_header as $table_head) { ?>
						<div class="d-flex justify-content-center align-items-center th <?php echo $columnClass; ?>"><?php echo $table_head; ?>
							<div style="display:none;">
					...
					$fcData = array();
						$scData = array();
						$data[$practise['id']]['isSecondColContentEditable'] = 0;
						if( !empty($practise['dependingpractise_answer']) && !empty($practise['typeofdependingpractice']) && $practise['typeofdependingpractice']!='set_full_view'  ) { 
							if( !empty($practise['dependingpractise_answer']) && !empty($practise['typeofdependingpractice']) && $practise['typeofdependingpractice']=='get_answers_put_into_quetions'  ) {
								
								if($practise['depending_practise_details']['question_type']=='match_answer'){
									$dependent_ans_array = explode(';',$practise['dependingpractise_answer'][0]);
									$temp = [];
									foreach ($dependent_ans_array as $key => $value) {
										if($value == " "){
											array_push($temp,"");
										}else{
									...
									$dependent_ans_array = $temp;
									foreach( $practise['depending_practise_details']['question'] as $key => $value){
										if($dependent_ans_array[$key] == "") continue;
										if( !empty($practise['depending_practise_details']['question'])   ){
											...
											array_push($fcData,$value); 
										}


										if( !empty($practise['depending_practise_details']['question_2'])   ){
											if(isset($dependent_ans_array[$key]) && $dependent_ans_array[$key]!=""){
												if(str_contains($practise['depending_practise_details']['question_2'][$dependent_ans_array[$key]], '#@')){

													$exploded_value = explode('#@', $practise['depending_practise_details']['question_2'][$dependent_ans_array[$key]]);
													$value = $exploded_value[1];
												}

												array_push($scData, $practise['depending_practise_details']['question_2'][$dependent_ans_array[$key]]); 
											}
										}
								...
								elseif($practise['depending_practise_details']['question_type']=='two_blank_table_speaking_up'){
								
									if(!empty( $practise['dependingpractise_answer'])){
										foreach( $practise['dependingpractise_answer'][0]['text_ans'][0] as $key => $value){
											if( 
												!empty($practise['dependingpractise_answer'][0]['text_ans'][1][$key][1]) && 
												(
													$practise['dependingpractise_answer'][0]['text_ans'][1][$key][1]=='true' ||
													$practise['dependingpractise_answer'][0]['text_ans'][1][$key][1]==1  || 
													$practise['dependingpractise_answer'][0]['text_ans'][1][$key][1]==true
												)
											){
												//$fcData[$fc] = $value['col_2'];
												$table_header_array=array();
												foreach($table_header as $thk=>$thv){
													array_push($table_header_array, trim($thv));
												}
												if(!in_array($value['col_1'], $table_header_array)){
													array_push( $fcData, $value['col_1']);
												}

												if(!in_array($value['col_2'], $table_header_array)){
													array_push( $scData, $value['col_2'] );
												} 
		...
		?>
			...
			<?php }?>
					...
					<ul class="list-inline list-buttons">
						<li class="list-inline-item">
							<button type="button" class="btn btn-primary blankTableBtn_dependacy_{{$practise['id']}}" data-pid="{{$practise['id']}}" data-is_save="0" >Save</button>
						</li>
						<li class="list-inline-item">
							<button type="button" class="btn btn-primary submitBtn blankTableBtn_dependacy_{{$practise['id']}}" data-pid="{{$practise['id']}}" data-is_save="1">Submit</button>
						</li>
					</ul>
					<input type="hidden" name="table_type" value="<?php echo $columnCount; ?>" />
				</form>
...
<script>
	...
	data27["{{$practise['id']}}"]= {"dependentpractice_ans": "{{$data[$practise['id']]['dependentpractice_ans']}}" };
	data27["{{$practise['id']}}"]["isRolePlayPractice"] = "{{$data[$practise['id']]['isRolePlayPractice']}}"
	data27["{{$practise['id']}}"]["is_dependent"] = "{{$data[$practise['id']]['is_dependent']}}"; 
	if(data27["{{$practise['id']}}"]["is_dependent"]==1){
		if(data27["{{$practise['id']}}"]["dependentpractice_ans"]==0 ){
...
<script>
	data27["{{$practise['id']}}"]["typeofdependingpractice"] = "{{ $data[$practise['id']]['typeofdependingpractice'] }}";
	data27["{{$practise['id']}}"]["dependant_practise_question_type"]  = "{{ $data[$practise['id']]['dependant_practise_question_type'] }}";
	if(data27["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_hide_show" || data27["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_parentextra" || data27["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_gen_que_double" || data27["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero_parentextra_get_questions_and_answers" || data27["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero" || data27["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_get_ans" || data27["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view"){
	  
		data27["{{$practise['id']}}"]["dependant_practise_task_id"] = "{{ $data[$practise['id']]['dependant_practise_task_id'] }}";
		data27["{{$practise['id']}}"]["dependant_practise_id"] = "{{ $data[$practise['id']]['setFullViewFromPreviousPractice'] }}";
		$(function () {
			$('.cover-spin').fadeIn();
		});
		if(data27["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){
			 
			// IF DEPENDENT PRACTICE BELONGS TO SAME TASK THEN USE BELOW CODE.
			if(data27["{{$practise['id']}}"]["dependant_practise_id"] !=""  ){
				
				
				 setTimeout(function(){ 
				 
					 data27["{{$practise['id']}}"]["prevHTML"] = $(document).find('.course-tab-content').find('#abc-'+data27["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();				 
				 
					$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).html(data27["{{$practise['id']}}"]["prevHTML"]);
					$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
					$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).find('ul.list-buttons').remove();
					$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).find('.alert').remove();
					$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).find('textarea').attr("name","test");
					if( data27["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero" ){
						if(data27["{{$practise['id']}}"]["dependant_practise_question_type"] == "reading_no_blanks_listening_speaking_down"  || data27["{{$practise['id']}}"]["dependant_practise_question_type"]== "single_speaking_up_writing" ) {
							$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).find('.audio-player').remove();
							$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).find('fieldset').remove();
						}
					} 
					$('.cover-spin').fadeOut();
				 }, 8000 )
			...
			var baseUrl = "{{url('/')}}";
			data27["{{$practise['id']}}"]["dependant_practise_topic_id"] = "{{ $data[$practise['id']]['dependant_practise_topic_id'] }}";
			data27["{{$practise['id']}}"]["dependentURL"] = baseUrl+'/topic/'+data27["{{$practise['id']}}"]["dependant_practise_topic_id"]+'/'+data27["{{$practise['id']}}"]["dependant_practise_task_id"]+'?n='+data27["{{$practise['id']}}"]["dependant_practise_id"];
			$.get(data27["{{$practise['id']}}"]["dependentURL"],  //
			function (dataHTML, textStatus, jqXHR) {  // success callback
				data27["{{$practise['id']}}"]["prevHTML"] = $(dataHTML).find('.course-tab-content').find('#abc-'+data27["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();
				$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).html(data27["{{$practise['id']}}"]["prevHTML"]);
				$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
				$(document).find(".showPreviousPractice_"+data27["{{$practise['id']}}"]["dependant_practise_id"]).find('ul.list-buttons, .alert, input:hidden').remove();
...
<?php
   ...
   {
		if($practise['id']=='15686410375d7f900d31378')
		{
	?>	  </div>
	<?php
		}
