            ...
            <div class="owl-carousel">
                @if(!empty($practise['options']))
                    @foreach($practise['options'] as $k => $value)
                        <div class="item">
                            <div class="table-slider-box text-center d-flex mt-3 pb-3">
                                @foreach($value as $x=> $val)
                                    ...
                                    ?>
                                    <div class="w-50 table-option {{ $border_class }} shadow mr-3">
                                    <a href="#!" id="{{$k}}_{{$x}}" data-pos="{{$x}}" style="">{{ $val }}</a>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                ...
                @if(!empty($item))
                    <li contenteditable="false">{{ str_replace("...", "", substr($item,2)) }}
                        <span class="" contenteditable="false">
                            ...
                            <input type="text" class="form-control text-left pl-0 form-control-inline text_question border-0" id="text_ans_{{$key}}" name="text_ans[{{$key}}][ans]" data-id="{{$key}}" value="{{ $answerExists == true ? $practise['user_answer'][0][$key]['ans'] : "" }}" >
    ...
    <div class="alert alert-danger" role="alert" style="display:none"></div>
        <ul class="list-inline list-buttons">
            <li class="list-inline-item">
                <input type="button" class="save_btn readingtotalblanks_speakingupBtn_{{$practise['id']}} btn btn-primary" value="Save" data-is_save="0">
            </li>
            <li class="list-inline-item">
                <input type="button" class="submit_btn readingtotalblanks_speakingupBtn_{{$practise['id']}} btn btn-primary" value="Submit" data-is_save="1">
            </li>
        </ul>
...
$(document).ready(function()
{
   ...
   var maxcounter={{ count($exploded_question) }}
    ...
    $('.table-option a').on('click', function(e){
        ...
        $('#text_ans_'+counter+'').val(selected_item).text(selected_item);
