...
if(empty($practise['dependingpractiseid']) &&  empty($practise['is_dependent']) ){
	...
	if(isset($practise['is_roleplay']) && $practise['is_roleplay'] == 1) { ?>
    	...
    	<div class="table-container">
		    <form class="save_two_blank_table_form" id="blank_table_form_<?php echo $practise['id'];?>">
		    	<input type="hidden" class="deleted_audio_{{$practise['id']}}_0" name="three_table_option_speaking" value="0">
		    	<?php
	...
	<script type="text/javascript">
		...
		$(document).on('click','.blankTableBtn_{{$practise["id"]}}' ,function() {
	        ...
	        $(".record_{{isset($practise['id'])?$practise['id']:''}}").each(function(){
            	if($(this).find('.audioplayer').hasClass('audioplayer-playing')){
		        ...
		        if($(this).attr('data-is_save') == '1'){
					...
					$('.main-audio-record-div').each(function(){
						...
						if($(this).find(".practice_audio").children().attr("src").indexOf("sample-audio.mp3") !== -1){
							...
							AllAudio.push($(this).find('fieldset').html())
						}else{
