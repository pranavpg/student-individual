...
if(isset($practise['user_answer']) && !empty($practise['user_answer'])){
	...
	//pr($roleplayOriginalPractiseAnswer);
...
<div class="table-container">
	  ...
	  <input type="hidden" name="active_rolecard" class="active_rolecard" value="">
	...
	<div class="two-click-content w-100">
		<?php foreach($rolePlayUsers as $c=>$rolePlayUser){
           ...
           // pr($exploded_question);
			...
			<?php 
				// Below code will get options
                if(isset($allOptions[$c]) && !empty($allOptions[$c])){
			...
			<?php }?>
			<?php
			// Below code will get tape script popup
			$exploded_question = explode(PHP_EOL,$rolePlayQuestions[$c+1]);
...
// dd($practise);
		...
		?>
			...
			<?php
							...
							if (isset($exploded_question[$columnsStartWith]) && intval(  $exploded_question[$columnsStartWith]))
							{
							    $firstColumns = array();
...
$(".selected_option").click(function() {
	...
	var content_key = $(this).attr('data-c');
	
	$('.active_rolecard').val(content_key);
