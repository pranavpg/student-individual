...
if(empty($practise['dependingpractiseid']) &&  empty($practise['is_dependent']) ){
	...
	if(isset($practise['is_roleplay']) && $practise['is_roleplay'] == 1){ ?>
    	...
    	<div class="table-container">
	        ...
	        <input type="hidden" class="deleted_audio_{{$practise['id']}}_0" name="four_blank_table_speaking_up" value="0">
				...
				<div class="table m-auto">
					...
					if(isset($totalColumnCounts)){
						...
						if(empty($table_header)){
							$border_color="border-top: 1px solid #e0e2e4;";
						}
						...
						<div class="table-row thead-dark d-flex justify-content-between">
						  <?php for ($k = 1;$k <= $columnCount;$k++) { ?>
								<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea" style="{{ ($j==0)?$border_color:''}}">
			...
			<?php }?>
					...
					<ul class="list-inline list-buttons">
						<li class="list-inline-item">
								...
								<input type="button" class="save_btn blankTableBtn_{{$practise['id']}} btn btn-primary" value="Save" data-is_save="0">
						</li>
						<li class="list-inline-item">
						<input type="button" class="submit_btn blankTableBtn_{{$practise['id']}} btn btn-primary" value="Submit" data-is_save="1">
...
?>
		...
		<div class="table-container">
			...
			<div class="table-container mb-4 text-center">
				<div class="table m-auto">
					...
					<div class="table-heading thead-dark d-flex justify-content-between">
						<?php foreach ($table_header as $table_head) { ?>
						<div class="d-flex justify-content-center align-items-center th <?php echo $tableHeaderColumnClass; ?>"><?php echo $table_head; ?></div>
						<div style="display:none">
							<textarea name="col[]"><?php echo $table_head; ?></textarea>
							<input type="hidden" name="true_false[]" value="false" />
						</div>
 ...
 -->					
					...
					<ul class="list-inline list-buttons">
				          <li class="list-inline-item">
				          		<button type="button" class="save_btn btn btn-primary submitBtn blankTableBtn_dependacy_{{$practise['id']}}" data-pid="{{$practise['id']}}" data-toggle="modal" data-is_save="0" data-target="#exitmodal" >Save</button>
				          </li>
				          <li class="list-inline-item"><button type="button" class="submit_btn btn btn-primary submitBtn blankTableBtn_dependacy_{{$practise['id']}}" data-pid="{{$practise['id']}}" data-is_save="1" >Submit</button>
				          </li>
				    </ul>
...
<script>
	...
	data20["{{$practise['id']}}"]= {"dependentpractice_ans": "{{$data[$practise['id']]['dependentpractice_ans']}}" };
	data20["{{$practise['id']}}"]["isRolePlayPractice"] = "{{$data[$practise['id']]['isRolePlayPractice']}}"
	data20["{{$practise['id']}}"]["is_dependent"] = "{{$data[$practise['id']]['is_dependent']}}"; 
	if(data20["{{$practise['id']}}"]["is_dependent"]==1){
		
		if(data20["{{$practise['id']}}"]["dependentpractice_ans"]==0 ){
...
<script>
	...
	data20["{{$practise['id']}}"]["typeofdependingpractice"] = "{{ $data[$practise['id']]['typeofdependingpractice'] }}";
	data20["{{$practise['id']}}"]["dependant_practise_question_type"]  = "{{ $data[$practise['id']]['dependant_practise_question_type'] }}";
	if(data20["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_hide_show" || data20["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_parentextra" || data20["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_gen_que_double" || data20["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero_parentextra_get_questions_and_answers" || data20["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero" || data20["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_get_ans" || data20["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view"){
	  
		data20["{{$practise['id']}}"]["dependant_practise_task_id"] = "{{ $data[$practise['id']]['dependant_practise_task_id'] }}";
		data20["{{$practise['id']}}"]["dependant_practise_id"] = "{{ $data[$practise['id']]['setFullViewFromPreviousPractice'] }}";
		$(function () {
			$('.cover-spin').fadeIn();
		});
		if(data20["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){
			 
			// IF DEPENDENT PRACTICE BELONGS TO SAME TASK THEN USE BELOW CODE.
			if(data20["{{$practise['id']}}"]["dependant_practise_id"] !=""  ){
				
				 
				 setTimeout(function(){ 
				 
					 data20["{{$practise['id']}}"]["prevHTML"] = $(document).find('.course-tab-content').find('#abc-'+data20["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();				 
				 
					$(document).find(".showPreviousPractice_"+data20["{{$practise['id']}}"]["dependant_practise_id"]).html(data20["{{$practise['id']}}"]["prevHTML"]);
					$(document).find(".showPreviousPractice_"+data20["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
					$(document).find(".showPreviousPractice_"+data20["{{$practise['id']}}"]["dependant_practise_id"]).find('ul.list-buttons').remove();
					$(document).find(".showPreviousPractice_"+data20["{{$practise['id']}}"]["dependant_practise_id"]).find('.alert').remove();

					if( data20["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero" ){
						if(data20["{{$practise['id']}}"]["dependant_practise_question_type"] == "reading_no_blanks_listening_speaking_down"  || data20["{{$practise['id']}}"]["dependant_practise_question_type"]== "single_speaking_up_writing" ) {
							$(document).find(".showPreviousPractice_"+data20["{{$practise['id']}}"]["dependant_practise_id"]).find('.audio-player').remove();
							$(document).find(".showPreviousPractice_"+data20["{{$practise['id']}}"]["dependant_practise_id"]).find('fieldset').remove();
						}
					} 
					$('.cover-spin').fadeOut();
				 }, 8000 )
			...
			var baseUrl = "{{url('/')}}";
			data20["{{$practise['id']}}"]["dependant_practise_topic_id"] = "{{ $data[$practise['id']]['dependant_practise_topic_id'] }}";
			data20["{{$practise['id']}}"]["dependentURL"] = baseUrl+'/topic/'+data20["{{$practise['id']}}"]["dependant_practise_topic_id"]+'/'+data20["{{$practise['id']}}"]["dependant_practise_task_id"]+'?n='+data20["{{$practise['id']}}"]["dependant_practise_id"];
			$.get(data20["{{$practise['id']}}"]["dependentURL"],  //
			function (dataHTML, textStatus, jqXHR) {  // success callback
				setTimeout(function(){
					data20["{{$practise['id']}}"]["prevHTML"] = $(dataHTML).find('.course-tab-content').find('#abc-'+data20["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();
					$(document).find(".showPreviousPractice_"+data20["{{$practise['id']}}"]["dependant_practise_id"]).html(data20["{{$practise['id']}}"]["prevHTML"]);
					$(document).find(".showPreviousPractice_"+data20["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
					$(document).find(".showPreviousPractice_"+data20["{{$practise['id']}}"]["dependant_practise_id"]).find('ul.list-buttons, .alert, input:hidden').remove();
