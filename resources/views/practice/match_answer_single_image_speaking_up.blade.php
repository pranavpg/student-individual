...
$(document).ready(function() {
  ...
  $(document).on('click','.submitBtn_{{$practise["id"]}}' ,function(e) {
    ...
    var reviewPopup = '{!!$reviewPopup!!}';
        ...
        if(markingmethod =="student_self_marking"){
            if($(this).attr('data-is_save') == '1'){
                ...
                // $("#selfMarking_{{isset($practise['id'])?$practise['id']:''}}").removeClass('component-two-click');
                $("#selfMarking_{{isset($practise['id'])?$practise['id']:''}}").find('#practise_div').html("");
                var fullView= $(".component-two-click").clone();
    ...
    // var speaking_practise = "{{($practise['type']=='match_answer_single_image_speaking_up')?1:0}}"
    ...
    // {
      ...
      $.ajax({
        url: '<?php echo URL('save-match-answer-single-image-speaking-up'); ?>',
