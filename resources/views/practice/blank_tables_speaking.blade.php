									...
									<?php for ($k = 1;$k <= $columnCount;$k++) {
										...
										<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea">
											...
											<?php } else {
												...
												?>
												<span class="textarea form-control form-control-textarea" role="textbox" {{$content_editable}} placeholder="Write here...">
													<?php
														if ($answerExists) {
														...
														//	echo '===>'.gettype($practice_user_ans[1][$j][$k-1]);die;
													?>
												</span>
												<div style="display:none">
...
$(document).on('click','.submitBtn_{{$practise["id"]}}' ,function() {
  ...
  setTextareaContent();
  $.ajax({
      url: '<?php echo URL('save-blank-table-speaking'); ?>',
      headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
      type: 'POST',
      data: $('.save_blank_table_speaking_form').serialize(),
