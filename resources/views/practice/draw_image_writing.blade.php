...
<?php
    ...
    if(!empty($load_image)){
        ...
        if($rawImage)
        {
            ...
            if(file_exists("./public/images/draw/".end($filename))){
              unlink("./public/images/draw/".end($filename));
             }
...
<script type="text/javascript">
    ...
    $(document).on('click',".draw_img_writing_email_form_{{$practise['id']}}" ,function() {
        ...
        var pid = "{{$practise['id']}}";
        //add static conition for level 0 ges topic 23 task 3
        if(pid !== "1628059320610a36b822fae")
        {
           setTextareaContent();
        }
        $.ajax({
