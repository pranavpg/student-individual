      ...
      <div class="two-click-content w-100">
        ...
        <?php $answer_count=0; ?>
          @php
             ...
             {
                $i = 0;
                foreach($practise['user_answer'] as $value)
                {
                   if($value !== "##")
                   {
                       $practise['user_answer'][$i] = $value; 
                       $i++;
                   } 
                }
             }
          ...
          @foreach($explode_question as $k => $v)
            <div class="content-box multiple-choice d-none selected_option_description selected_option_description_{{$k}}" >
             ...
             ?>
              <input type="hidden" name="split_count[{{$k}}]" value="{{$answer_count}}">
...
<script>
  ...
  $(document).on('click','.submitBtn_{{$practise["id"]}}' ,function() {
    ...
    setTextareaContent();
    $.ajax({
        url: '<?php echo URL('save-speaking-multiple-single-image-roleplay'); ?>',
