									  ...
									  <?php for ($k = 1;$k <= $columnCount;$k++) { ?>
											<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea" >
												...
												<?php }else { 
															...
															<span class="textarea form-control form-control-textarea  {{$disaplyAns}}" role="textbox" contenteditable placeholder="Write here..." style="">{!!$disaplyAns!!}</span>
															<div style="display:none">
																<textarea name="col[]">
																	<?php
															            if ($answerExists) {
	...
	<script type="text/javascript">
		...
		$(document).on('click','.blankTableBtn_{{$practise["id"]}}' ,function() {
			...
			$("span.textarea.form-control").each(function(){
		  	...
		  	$.ajax({
		      	url: '<?php echo URL('save-blank-table-three-table-option'); ?>',
	    ...
	    <script type="text/javascript">
	    			$(document).on('click','.blankTableBtn_dependacy_{{$practise["id"]}}' ,function() {
						...
						$("span.textarea.form-control").each(function(){
					  	...
					  	$.ajax({
					      	url: '<?php echo URL('save-blank-table-three-table-option-dependancy'); ?>',
					      	headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
					      	type: 'POST',
					      	data: $('#blank_table_form_<?php echo $practise['id'];?>').serialize(),
					        success: function (data) {
