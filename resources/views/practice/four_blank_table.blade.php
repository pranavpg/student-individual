						...
						<div class="table-row thead-dark d-flex justify-content-between">
						  <?php for ($k = 1;$k <= $columnCount;$k++) { ?>
								<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea">
									...
									<?php }elseif ($k == 3 && isset($thirdColumns) && !empty($thirdColumns) && !empty($thirdColumns[$j]) && trim($thirdColumns[$j])!="") { $rowflag=0; ?>
									...
									<?php }elseif ($k == 4 && isset($fourColumns) && !empty($fourColumns) && !empty(trim($fourColumns[$j]))) { $rowflag=0; ?>
			...
			<div class="table-container mb-4 text-center">
				<div class="table m-auto">
					...
					if( !empty($practise['dependingpractise_answer']) && !empty($practise['typeofdependingpractice']) && $practise['typeofdependingpractice']=='get_answers_put_into_quetions' && empty($firstColumns) ) {
						...
						// foreach( $practise['depending_practise_details'][0][0] as $key => $value){
						...
						$firstColumns = $practise['depending_practise_details']['question'];
					}
					//print_r($firstColumns);
					//pr($practise);
					if(isset($totalColumnCounts)){
		...
		if(data13["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){
			...
			if(data13["{{$practise['id']}}"]["dependant_practise_id"] !=""  ){
				 ...
				 setTimeout(function(){ 
					 ...
					 data13["{{$practise['id']}}"]["prevHTML"] = $(document).find('.course-tab-content').find('#abc-'+data13["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();				 
