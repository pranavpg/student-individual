 ...
 @else
	...
	@else
		...
		@if(isset($practise['typeofdependingpractice']) && $practise['typeofdependingpractice'] == 'get_answers_put_into_quetions_before')
			...
			@endif
		@endif
		@if(isset($practise['typeofdependingpractice']) && ($practise['typeofdependingpractice'] == 'set_full_view'  || $practise['typeofdependingpractice'] == 'set_full_view_remove_zero'))
				...
				@if(!empty($value))
					...
					@endif
						<div class="form-group d-flex align-items-start form-group-label">
							<span class="textarea form-control form-control-textarea enter_disable" role="textbox" contenteditable placeholder="Write here..." style="color:black">
									<?php
										if ($answerExists)
										{
...
/*
		...
		$(document).on('click','.speakingMultipleBtn_{{$practise["id"]}}' ,function() {
					...
					// if($('.record_'+$(this).attr("data-pid")+' .audioplayer').hasClass("audioplayer-playing")){
		                ...
		                $('.audioplayer').each(function(){
	                		if($(this).hasClass('audioplayer-playing')){
								$(this).find('.audioplayer-playpause').click()	
	                		}
		                });
		            // }
					...
					if(markingmethod =="student_self_marking"){
						if($(this).attr('data-is_save') == '1'){
							...
							/*$(".form_{{$practise['id']}}").find('.audioplayer-playpause').each(function(){
		...
		data34["{{$practise['id']}}"]= {"dependentpractice_ans": "{{$data[$practise['id']]['dependentpractice_ans']}}" };
		data34["{{$practise['id']}}"]["isRolePlayPractice"] = "{{$data[$practise['id']]['isRolePlayPractice']}}"
		data34["{{$practise['id']}}"]["is_dependent"] = "{{$data[$practise['id']]['is_dependent']}}"; 
	</script>

	
@if(!empty($practise['is_dependent']) && $practise['is_dependent']==1 && $data[$practise['id']]['isRolePlayPractice']==0 && ($practise['typeofdependingpractice']=='set_full_view' || $practise['typeofdependingpractice']=='set_full_view_remove_zero' ))
<script>


	data34["{{$practise['id']}}"]["typeofdependingpractice"] = "{{ $data[$practise['id']]['typeofdependingpractice'] }}";
	data34["{{$practise['id']}}"]["dependant_practise_question_type"]  = "{{ $data[$practise['id']]['dependant_practise_question_type'] }}";
	if(data34["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_hide_show" || data34["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_parentextra" || data34["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_gen_que_double" || data34["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero_parentextra_get_questions_and_answers" || data34["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero" || data34["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_get_ans" || data34["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view"){
		
		data34["{{$practise['id']}}"]["dependant_practise_task_id"] = "{{ $data[$practise['id']]['dependant_practise_task_id'] }}";
		data34["{{$practise['id']}}"]["dependant_practise_id"] = "{{ $data[$practise['id']]['setFullViewFromPreviousPractice'] }}";
		$(function () {
			$('.cover-spin').fadeIn();
		});
		if(data34["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){
			
			// IF DEPENDENT PRACTICE BELONGS TO SAME TASK THEN USE BELOW CODE.
			if(data34["{{$practise['id']}}"]["dependant_practise_id"] !=""  ){
				
				 
				 setTimeout(function(){ 
					// alert( $(document).find('.course-tab-content').find('#abc-'+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html())
					 data34["{{$practise['id']}}"]["prevHTML"] = $(document).find('.course-tab-content').find('#abc-'+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();				 
				 
					$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).html(data34["{{$practise['id']}}"]["prevHTML"]);
					$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
					$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('ul.list-buttons').remove();
					$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('.alert').remove();

					if( data34["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero" ){
						if(data34["{{$practise['id']}}"]["dependant_practise_question_type"] == "reading_no_blanks_listening_speaking_down"  || data34["{{$practise['id']}}"]["dependant_practise_question_type"]== "single_speaking_up_writing" ) {
							$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('.audio-player').remove();
							$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('fieldset').remove();
						}

					} 

					if( data34["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero" ){
						$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('.topic_id').remove();
						$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('.task_id').remove();
						$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('.is_save').remove();
						$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('.practise_id').remove();
						$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('.practice_type').remove();
						$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('.draw-image').remove();
					}
					$('.cover-spin').fadeOut();
				 }, 8000 )
			}
			...
			var baseUrl = "{{url('/')}}";
			data34["{{$practise['id']}}"]["dependant_practise_topic_id"] = "{{ $data[$practise['id']]['dependant_practise_topic_id'] }}";
			data34["{{$practise['id']}}"]["dependentURL"] = baseUrl+'/topic/'+data34["{{$practise['id']}}"]["dependant_practise_topic_id"]+'/'+data34["{{$practise['id']}}"]["dependant_practise_task_id"]+'?n='+data34["{{$practise['id']}}"]["dependant_practise_id"];
			$.get(data34["{{$practise['id']}}"]["dependentURL"],  //
			function (dataHTML, textStatus, jqXHR) {  // success callback
				setTimeout(function(){
		
					data34["{{$practise['id']}}"]["prevHTML"] = $(dataHTML).find('.course-tab-content').find('#abc-'+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();
					$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).html(data34["{{$practise['id']}}"]["prevHTML"]);
					$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
					$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('ul.list-buttons, .alert, input:hidden').remove();
					$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('textarea').attr("name","new");
					
					if(data34["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero" ){
						
						if(data34["{{$practise['id']}}"]["dependant_practise_question_type"] == "reading_no_blanks_listening_speaking_down" || data34["{{$practise['id']}}"]["dependant_practise_question_type"]== "single_speaking_up_writing"|| data34["{{$practise['id']}}"]["dependant_practise_question_type"]== "writing_at_end_speaking_up") {
							$(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('fieldset').remove();
						}
							// $(document).find(".showPreviousPractice_"+data34["{{$practise['id']}}"]["dependant_practise_id"]).find('fieldset').remove();
					...
					Audioplay(data34["{{$practise['id']}}"]["dependant_practise_id"])
				},8000)
		...
		function Audioplay(oldid){
			var supportsAudio = !!document.createElement('audio').canPlayType;

			
			$(".answer_audio-"+oldid+"-"+0).find('.plyr__controls__item').fadeOut();

			if($('.record_'+oldid+' .answer_audio-'+oldid+'-0').children().attr("src").indexOf("sample-audio.mp3") !== -1){
				if (supportsAudio) {
					var i;
					var player = new Plyr(".answer_audio-"+oldid+"-"+0, {
						controls: [
							'progress',
							'current-time'
						]
				...
				$('.record_'+oldid+' .plyr__progress__container').css("position","initial")
				$('.record_'+oldid+' .plyr__progress__container').css("height","19px")
				$('.record_'+oldid+' .plyr__progress__container').css("padding","5px")
				$('.record_'+oldid+' .plyr__progress__container').css("background-color","#e9ecef")
