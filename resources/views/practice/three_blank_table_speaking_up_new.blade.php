...
if(empty($practise['dependingpractiseid']) &&  empty($practise['is_dependent']) ){
	...
	if(isset($practise['is_roleplay']) && $practise['is_roleplay'] == 1){ ?>
		@include('practice.blank_tables_roleplay_three') <?php 
	} else {
		...
		<?php }?>
	        ...
	        <input type="hidden" class="deleted_audio_{{$practise['id']}}_0" name="three_blank_table_speaking_up_new" value="0">
    		<?php
				...
				$columnCount = 1;
				if ($practise['type'] == "three_blank_table" || $practise['type'] == "three_blank_table_listening" || $practise['type'] == "three_blanks_table_listening" || $practise['type'] == "three_table_option" || $practise['type'] == "three_blank_table_speaking" || $practise['type'] == "three_table_option_speaking" || $practise['type'] == "three_blank_table_speaking_up" || $practise['type'] == "three_table_option_listening")
				{
				    $columnCount = 3;
				    $columnClass = 'w-33';
				}
	...
	<script type="text/javascript">
		...
		$(document).on('click','.blankTableBtn_{{$practise["id"]}}' ,function() {
			...
			$("span.textarea.form-control").each(function(){
		  	...
		  	$.ajax({
		      	url: '<?php echo URL('save-blank-table-three-blanck-table'); ?>',
	    ...
	    <script type="text/javascript">
	    			$(document).on('click','.blankTableBtn_dependacy_{{$practise["id"]}}' ,function() {
						...
						$("span.textarea.form-control").each(function(){
					  	...
					  	$.ajax({
					      	url: '<?php echo URL('save-blank-table-dependancy-three-blank-table-speaking-up-new'); ?>',
					      	headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
					      	type: 'POST',
					      	data: $('#blank_table_form_<?php echo $practise['id'];?>').serialize(),
					        success: function (data) {
