...
{{--<!-- <div class="two-click d-flex flex-wrap justify-content-center mb-4 w-100">
              ...
              <div class="two-click d-flex flex-wrap justify-content-center mb-4 w-100">
                ...
                <div class="two-click-content w-100">
                    ...
                    <div class="content-box multiple-choice d-none selected_option_description selected_option_description_{{$key}}">
                                ...
                                <p>
                                    <div class="form-slider p-0 mb-4">
                                        <div class="component-control-box">
                                            <span class="textarea form-control form-control-textarea set_full_view_main stringProper text-left" role="textbox" contenteditable="" placeholder="Write here..." >{!! isset($practise['user_answer'][$roleplayKey]) ? nl2br($practise['user_answer'][$roleplayKey]) : ''!!}</span>
                            ...
                            <?php 
                             if($practise['id'] == '15569122135ccc98557adb4')
                             {
                              //---add condition for temp purpose 
                            ?>
                               <div id="defined_dependant_task_{{$practise['id']}}" style="display:{{$displayDependBox}}; border: 2px dashed gray; border-radius: 12px;">
                                <p style="margin: 15px;">In order to do this task you need to have completed <strong>  <?php if(isset($depend[2])){ echo 'Task '.$depend[2].' ';}?>Practice <?php if($depend[3] == 1){echo "A";} if($depend[3] == 2){echo "B";}if($depend[3] == 3){echo "C";}if($depend[3] == 4){echo "D";}if($depend[3] == 5){echo "E";} if($depend[3] == 6){echo "F";} ?>. </strong> Please complete this first.</p>
                               </div>
                            <?php  
                             }
                             else
  ...
  <ul class="list-inline list-buttons">
      <li class="list-inline-item"><button type="button" class="save_btn btn btn-primary saveBtn listeningWritingBtn_{{$practise['id']}}" data-pid="{{$practise['id']}}"
          data-toggle="modal" data-is_save="0" data-target="#exitmodal">Save</button></li>
      <li class="list-inline-item"><button type="button"
          class="submit_btn btn btn-primary submitBtn listeningWritingBtn_{{$practise['id']}}" data-pid="{{$practise['id']}}" data-is_save="1">Submit</button></li>
  </ul>
</form>
...
<script>
  ...
  var token = $('meta[name=csrf-token]').attr('content');
	...
	data6["{{$practise['id']}}"]= {"dependentpractice_ans": "{{$data[$practise['id']]['dependentpractice_ans']}}" };
	data6["{{$practise['id']}}"]["isRolePlayPractice"] = "{{$data[$practise['id']]['isRolePlayPractice']}}"
	data6["{{$practise['id']}}"]["is_dependent"] = "{{$data[$practise['id']]['is_dependent']}}"; 
	// if(data6["{{$practise['id']}}"]["is_dependent"]==1){
		
	// 	if(data6["{{$practise['id']}}"]["dependentpractice_ans"]==0 ){
    ...
    $(function() {
        data6["{{$practise['id']}}"]["dependent_is_roleplay"] = "{{$data[$practise['id']]['dependent_is_roleplay']}}";
        ...
        $(".save_listening_writing_form_{{$practise['id']}}").find(".selected_option").click(function() {
          if($(this).attr('data-is_save') == '1'){
            ...
            if ($(".save_listening_writing_form_{{$practise['id']}}").find('.selected_option_description:visible').length > 0) {
                if(data6["{{$practise['id']}}"]["dependent_is_roleplay"] && show_dependent_error!=1){
    ...
    function roleplayDependent(content_key, answer_count){
      ...
      data6["{{$practise['id']}}"]["typeofdependingpractice"] = "{{$data[$practise['id']]['typeofdependingpractice']}}";
      data6["{{$practise['id']}}"]["dependant_practise_question_type"]  = "{{$data[$practise['id']]['dependant_practise_question_type']}}";
      if(data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_hide_show" || data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_parentextra" || data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_gen_que_double" || data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero_parentextra_get_questions_and_answers" || data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero" || data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_get_ans" || data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view" || data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero_get_single_audio"){
        data6["{{$practise['id']}}"]["dependant_practise_task_id"] =  "{{$data[$practise['id']]['dependant_practise_task_id']}}";
        data6["{{$practise['id']}}"]["dependant_practise_id"] = "{{$data[$practise['id']]['setFullViewFromPreviousPractice']}}";
        $(function () {
            // $('.cover-spin').fadeIn();
        })
        if(data6["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{ request()->segment(3)}}"){
          
          // IF DEPENDENT PRACTICE BELONGS TO SAME TASK THEN USE BELOW CODE.
          if(data6["{{$practise['id']}}"]["dependant_practise_id"] !="" ){

            setTimeout(function(){ 
               
              $(".showPreviousPractice_{{$practise['id']}}").html("");
              if(data6["{{$practise['id']}}"]["dependent_is_roleplay"] == 1 ){
            
                var prevHTML = $(document).find('.course-tab-content').find('#abc-'+data6["{{$practise['id']}}"]["dependant_practise_id"]).find('form').find( '.selected_option_description_'+answer_count).html();	
                if(prevHTML==undefined){
                  var prevHTML = $(document).find('.course-tab-content').find('#abc-'+data6["{{$practise['id']}}"]["dependant_practise_id"]).find('form').find( '#table_dependent_'+answer_count).html();	
                }			 
              } else {
                var prevHTML = $(document).find('.course-tab-content').find('#abc-'+data6["{{$practise['id']}}"]["dependant_practise_id"]).find('form').find( '.selected_option_description_'+content_key ).html();				 
              }
              ...
              if(data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero"){
                if( data6["{{$practise['id']}}"]["dependant_practise_question_type"] == "writing_at_end_speaking_up" ||  data6["{{$practise['id']}}"]["dependant_practise_question_type"] == "single_speaking_up_writing") {
                  $(".showPreviousPractice_{{$practise['id']}}").find('fieldset').remove();
                }
              }
              if(data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero_parentextra_get_questions_and_answers"){
                if( data6["{{$practise['id']}}"]["dependant_practise_question_type"] == "writing_at_end_speaking_up" ||  data6["{{$practise['id']}}"]["dependant_practise_question_type"] == "single_speaking_up_writing") {
                  $(".showPreviousPractice_{{$practise['id']}}").find('fieldset').remove();
          ...
          data6["{{$practise['id']}}"]["dependant_practise_topic_id"] = "{{$data[$practise['id']]['dependant_practise_topic_id']}}";
          var dependentURL = baseUrl+'/topic/'+data6["{{$practise['id']}}"]["dependant_practise_topic_id"]+'/'+data6["{{$practise['id']}}"]["dependant_practise_task_id"]+'?n='+data6["{{$practise['id']}}"]["dependant_practise_id"];
          // console.log(data);
          $.get(dependentURL,function (dataHtml, textStatus, jqXHR) {
            // console.log(dataHtml);
            // alert('#abc-'+data6["{{$practise['id']}}"]["dependant_practise_id"]);  // success callback
            var prevHTML = $(dataHtml).find('.course-tab-content').find('#abc-'+data6["{{$practise['id']}}"]["dependant_practise_id"]).find('form').find('.selected_option_description_'+content_key).html();				 
             //alert(prevHTML);
            $('.showPreviousPractice_{{$practise["id"]}}').html(prevHTML);
            $(".showPreviousPractice_{{$practise['id']}}").css('pointer-events','none');
            $(".showPreviousPractice_{{$practise['id']}}").find('ul.list-buttons').remove();
            if(data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero_get_single_audio"){   
              if(data6["{{$practise['id']}}"]["dependant_practise_question_type"] == "writing_at_end_speaking_up" ||  data6["{{$practise['id']}}"]["dependant_practise_question_type"] == "single_speaking_up_writing") {
                $(".showPreviousPractice_{{$practise['id']}}").find('fieldset').remove();
              }
              if(data6["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero_get_single_audio"){
