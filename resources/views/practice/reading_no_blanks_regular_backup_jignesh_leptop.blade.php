	      	...
	      	<form class="reading-no-blanks_form" id="reading-no-blanks_form-<?php echo $practise['id'];?>">
				...
				if($practise['typeofdependingpractice'] == "set_full_view_remove_top_zero" && $practise['depending_practise_details']['question_type'] == "four_table_option"){
					  ...
					  <script>
							...
							data21["{{$practise['id']}}"]= {"dependentpractice_ans": "{{$data[$practise['id']]['dependentpractice_ans']}}" };
							data21["{{$practise['id']}}"]["isRolePlayPractice"] = "{{$data[$practise['id']]['isRolePlayPractice']}}"
							data21["{{$practise['id']}}"]["is_dependent"] = "{{$data[$practise['id']]['is_dependent']}}"; 
							if(data21["{{$practise['id']}}"]["is_dependent"]==1){
								
								if(data21["{{$practise['id']}}"]["dependentpractice_ans"]==0 ){
						...
						@if(!empty($practise['is_dependent']) && $practise['is_dependent']==1 && $data[$practise['id']]['isRolePlayPractice']==0 && $practise['typeofdependingpractice']!='get_answers_generate_quetions' )
							<script>
								...
								data21["{{$practise['id']}}"]["typeofdependingpractice"] = "{{ $data[$practise['id']]['typeofdependingpractice'] }}";
								data21["{{$practise['id']}}"]["dependant_practise_question_type"]  = "{{ $data[$practise['id']]['dependant_practise_question_type'] }}";
								if(data21["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_hide_show" || data21["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_parentextra" || data21["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_gen_que_double" || data21["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero_parentextra_get_questions_and_answers" || data21["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero" || data21["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_get_ans" || data21["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view" || data21["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_top_zero"){
								  
									data21["{{$practise['id']}}"]["dependant_practise_task_id"] = "{{ $data[$practise['id']]['dependant_practise_task_id'] }}";
									data21["{{$practise['id']}}"]["dependant_practise_id"] = "{{ $data[$practise['id']]['setFullViewFromPreviousPractice'] }}";
							        $(function () {
							            // $('.cover-spin').fadeIn();
							        });
									if(data21["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){
							             
										// IF DEPENDENT PRACTICE BELONGS TO SAME TASK THEN USE BELOW CODE.
										if(data21["{{$practise['id']}}"]["dependant_practise_id"] !=""  ){
							                setTimeout(function(){ 
							                    data21["{{$practise['id']}}"]["prevHTML"] = $(document).find('.course-tab-content').find('#abc-'+data21["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();				 
											    $(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).html(data21["{{$practise['id']}}"]["prevHTML"]);
												$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
												$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find('.removebutton').remove();
												$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find('.alert').remove();
												if( data21["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero"){
														$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find('.audio-player').remove();

													if(data21["{{$practise['id']}}"]["dependant_practise_question_type"] == "writing_at_end_speaking_up") {
														$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find('fieldset').remove();
							                        }
							                        if(data21["{{$practise['id']}}"]["dependant_practise_question_type"] == "set_in_order_vertical_listening") {
														$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find('.audio-player').remove();
													}
												}
							                    // $('.cover-spin').fadeOut();
										...
										var baseUrl = "{{url('/')}}";
										data21["{{$practise['id']}}"]["dependant_practise_topic_id"] = "{{ $data[$practise['id']]['dependant_practise_topic_id'] }}";
										data21["{{$practise['id']}}"]["dependentURL"] = baseUrl+'/topic/'+data21["{{$practise['id']}}"]["dependant_practise_topic_id"]+'/'+data21["{{$practise['id']}}"]["dependant_practise_task_id"]+'?n='+data21["{{$practise['id']}}"]["dependant_practise_id"];
										$.get(data21["{{$practise['id']}}"]["dependentURL"],  //
										function (dataHTML, textStatus, jqXHR) {  // success callback
											data21["{{$practise['id']}}"]["prevHTML"] = $(dataHTML).find('.course-tab-content').find('#abc-'+data21["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();
											$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).html(data21["{{$practise['id']}}"]["prevHTML"]);
											$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
											$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find(".blankTableBtn_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).remove();
											$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find(".topic_id").remove();
											$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find(".task_id").remove();
											$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find(".is_save").remove();
											$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find(".practise_id").remove();
											$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find(".alert-success").remove();
											$(document).find(".showPreviousPractice_reding_"+data21["{{$practise['id']}}"]["dependant_practise_id"]).find(".alert-danger").remove();
