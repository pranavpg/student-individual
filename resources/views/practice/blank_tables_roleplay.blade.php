...
if(isset($practise['user_answer']) && !empty($practise['user_answer'])){
	...
	//pr($practise['user_answer'][1]['path']);
}else{
	$originalPractiseAnswer = array();
}
?>
<div class="table-container">
      <form class="save_two_blank_table_form" id="blank_table_form_roleplay_<?php echo $practise['id'];?>">
...
<div class="component-two-click mb-4">
	<div class="two-click d-flex flex-wrap justify-content-center mb-4 w-100">
		...
		foreach($rolePlayUsers as $c=>$rolePlayUser){?>
			<a href="javascript:;" id="s-button-<?php echo $c;?>" class="btn btn-dark s-button selected_option selected_option_{{$c}}" data-c="{{$c}}"><?php echo trim($rolePlayUser);?></a>
		<?php }?>
	</div>
	<div class="two-click-content w-100">
		<?php foreach($rolePlayUsers as $c=>$rolePlayUser){
			...
			if($checkFirstTwoChars !== "--"){
				...
				/*if(isset($practise['user_answer']) && !empty($practise['user_answer'])){
					...
					$newMakeAnswer = array();
					$newMakeAnswer[0][0] = $practise['user_answer'][0]['text_ans'][0];
					$newMakeAnswer[0][1] = $practise['user_answer'][0]['text_ans'][1];
			...
			<?php if($practise['type'] == "one_blank_table_speaking_up" || $practise['type'] == "two_blank_table_speaking_up" || $practise['type'] == "three_blank_table_speaking_up" || $practise['type'] == "four_blank_table_speaking_up" || $practise['type'] == "five_blank_table_speaking_up" || $practise['type'] == "six_blank_table_speaking_up" || $practise['type'] == "seven_blank_table_speaking_up" || $practise['type'] == "eight_blank_table_speaking_up" || $practise['type'] == "nine_blank_table_speaking_up" || $practise['type'] == "ten_blank_table_speaking_up" || $practise['type'] == "one_table_option_speaking_up" || $practise['type'] == "two_table_option_speaking_up" || $practise['type'] == "three_table_option_speaking_up" || $practise['type'] == "four_table_option_speaking_up" || $practise['type'] == "five_table_option_speaking_up" || $practise['type'] == "six_table_option_speaking_up" || $practise['type'] == "seven_table_option_speaking_up" || $practise['type'] == "eight_table_option_speaking_up" || $practise['type'] == "nine_table_option_speaking_up" || $practise['type'] == "ten_table_option_speaking_up"){ ?>
				<input type="hidden" name="audio_path[{{$c}}]" class="audio_path{{$c}}" value="">
			@include('practice.common.audio_record_div',['key'=>$c,'ss'=>$c])
			 <input type="hidden" class="deleted_audio_{{$practise['id']}}_{{$c}}" name="two_table_option_speaking_up_{{$c}}" value="0">
        ...
        // dd($practise);
				...
				foreach($rolePlayUsers as $rolePlayUser){
					if($speakingPractise){
						if(isset( $roleplayOriginalPractiseAnswer[$cc]['text_ans'][0])){
						$newMakeAnswer[$ccc][0] = $roleplayOriginalPractiseAnswer[$cc]['text_ans'][0];							
						}else{
							$newMakeAnswer[$ccc][0]="";
						}
					  ...
					  <?php for ($k = 1;$k <= $columnCount;$k++) { ?>
							<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea">
								...
								<?php }else{?>
									...
									<span class="textarea form-control form-control-textarea stringProper text-left" role="textbox" contenteditable placeholder="Write here..."><?php if ($answerExists) {
...
<?php }?>
		...
		<ul class="list-inline list-buttons">
				<li class="list-inline-item"><button type="button" class="save_btn btn btn-primary twoBlankTableBtn_{{$practise['id']}}" data-pid="{{$practise['id']}}"
								data-toggle="modal" data-is_save="0" data-target="#exitmodal">Save</button>
				</li>
				<li class="list-inline-item"><button type="button"
								class="submit_btn btn btn-primary submitBtn twoBlankTableBtn_{{$practise['id']}}" data-pid="{{$practise['id']}}" data-is_save="1">Submit</button>
				</li>
		</ul>
...
$(".selected_option").click(function() {
		...
		if ( $('#blank_table_form_roleplay_{{$practise["id"]}}').find('.two-click-content').find('.d-none').length ==2) {
...
<script type="text/javascript">
$(document).on('click','.twoBlankTableBtn_{{$practise["id"]}}',function() {
  $('.twoBlankTableBtn_{{$practise["id"]}}').attr('disabled','disabled');
  ...
  setTextareaContent();
  $.ajax({
      url: '<?php echo URL('save-two-blank-table-option-speaking'); ?>',
      headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
      type: 'POST',
      data: $('#blank_table_form_roleplay_<?php echo $practise['id'];?>').serialize(),
      success: function (data) {
        	$('.twoBlankTableBtn_{{$practise["id"]}}').removeAttr('disabled');
					if(data.success){
						$('.alert-danger').hide();
...
$(function () {
	// two click js
	$("#blank_table_form_roleplay_<?php echo $practise['id'];?> .s-button").on("click",function(){
		if($("#blank_table_form_roleplay_<?php echo $practise['id'];?> .s-button.d-none").length > 0){
			$("#blank_table_form_roleplay_<?php echo $practise['id'];?> .s-button.d-none");
			$("#blank_table_form_roleplay_<?php echo $practise['id'];?> .s-button-box").addClass("d-none");
			$("#blank_table_form_roleplay_<?php echo $practise['id'];?> .s-button").removeClass("d-none").removeClass("btn-bg");
			return false;
		}
		$("#blank_table_form_roleplay_<?php echo $practise['id'];?> .s-button-box,.s-button").addClass("d-none");
		$(this).removeClass("d-none").addClass("btn-bg");
		var curId = $(this).attr("id");
		curId  = curId.replace("s-button-","");
		$("#blank_table_form_roleplay_<?php echo $practise['id'];?> #s-button-"+curId+curId).removeClass("d-none");
	})
