...
if(empty($practise['dependingpractiseid']) &&  empty($practise['is_dependent']) ){
	...
	if(isset($practise['is_roleplay']) && $practise['is_roleplay'] == 1){ ?>
    	...
    	<div class="table-container">
            ...
            <input type="hidden" class="deleted_audio_{{$practise['id']}}_0" name="two_blank_table_speaking" value="0">
			...
			@if($practise['id'] == "15508226105c6facd2a884d")
				<div class="table-container mb-4 text-center" style="border-top: 1px solid #dfe2e5;">
			@else
				<div class="table-container mb-4 text-center" style="border-top: 1px solid #dfe2e5;">
			@endif
...
?>
		<div class="table-container">
    		...
    		<?php
				if($practise['type'] == "two_blank_table_up_writing_at_end_up" || $practise['type'] == "writing_at_end_four_blanks_table" || $practise['type'] == "writing_at_end_up_four_blanks_table"){
					...
					if(str_contains($practise['depending_practise_details']['question1'][0],'#@')){
						 $questionImpload = implode("@@",$practise['depending_practise_details']['question1']);
						$exploded_question = explode('#@', $questionImpload);
					}
					// $exploded_question = $practise['depending_practise_details']['question1'];
				}
				$checkFirstTwoChars = substr($exploded_question[0],0, 2);
				if($checkFirstTwoChars !== "--"){
					//pr($practise);die;
				...
				if (isset($practise['dependingpractise_answer']) && !empty($practise['dependingpractise_answer']))
				{
					...
					if(isset($practise['dependingpractise_answer'][0]) && !empty($practise['dependingpractise_answer'][0])){
						if(isset($practise['depending_practise_details']['question2']) && !empty($practise['depending_practise_details']['question2'])){
							$dependAns= explode(';',$practise['dependingpractise_answer'][0]);
							// pr($dependAns);
							foreach($dependAns as $ans){
								if($ans == 0){
									$dependFinalAnswer[]=$practise['depending_practise_details']['question2'][0];
								}
								 if(!empty((int)$ans)){
									$dependFinalAnswer[]=$practise['depending_practise_details']['question2'][$ans];
					...
					$dependFinalAnswer=array_unique($dependFinalAnswer);
				}


					//  dd($dependFinalAnswer);
				$flag = "";
				if (isset($exploded_question[$columnsStartWith]) && is_numeric($exploded_question[$columnsStartWith]))
				{
					 dd("sssss");
						  ...
						  <?php for ($k = 1;$k <= $columnCount;$k++) { ?>
								<div class="d-flex justify-content-center align-items-center  border-left td <?php echo $columnClass; ?> td-textarea">
									...
									<?php }else {
										...
										if(isset($dependFinalAnswer[$dependAnswerIncrement])){
																	echo $dependFinalAnswer[$dependAnswerIncrement];
																} ?>
												<input type="hidden" name="true_false[]" value="false" />
												<div style="display:none">
													<textarea name="col[]">
														<?php
												            if ($answerExists) {
												                // if(isset($practise['dependingpractise_answer'][0][0][$newJ]['col_' . $k])){
																// 					echo $practise['dependingpractise_answer'][0][0][$newJ]['col_' . $k];
																// 				} else if(!empty($practise['dependingpractise_answer'][0]['text_ans'][0][$newJ]['col_' . $k])){
																// 						echo $practise['dependingpractise_answer'][0]['text_ans'][0][$newJ]['col_' . $k];
																// 				}
																if(isset($dependFinalAnswer[$dependAnswerIncrement])){
																	echo $dependFinalAnswer[$dependAnswerIncrement++];
																}
		...
		@include('practice.common.audio_record_div',['key'=>0])
			...
			<?php }?>
					...
					<div class="alert alert-success" role="alert" style="display:none"></div>
					<div class="alert alert-danger" role="alert" style="display:none"></div>
	       			 	<input type="button" class="save_btn blankTableBtn_dependacy_{{$practise["id"]}} btn btn-primary" value="Save" data-is_save="0">
	        			<input type="button" class="submit_btn blankTableBtn_dependacy_{{$practise["id"]}} btn btn-primary" value="Submit" data-is_save="1">
					<input type="hidden" name="table_type" value="<?php echo $columnCount; ?>" />
				</form>
	    </div>
	    <?php
