        ...
        //  pr($practise['question_2']);
          ...
          @foreach($q1 as $key => $value)
            @if(!empty(trim($value)))
              ...
              <li class="list-item col-12 col-md-6 bg-list-light-gray answer_option {{($answerExists)?$background_color_class[$k]:''}} answer_option_{{$key}}" >
                <strong>{!! !empty($q2[$key])?$q2[$key]:"" !!}</strong>
                <input type="hidden" name="text_ans[]">
              </li>
            @endif
          @endforeach
        </ul>
