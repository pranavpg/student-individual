...
if(!isset($practise['dependingpractiseid'])) {
	...
	<form class="reading-no-blanks_form commonFontSize" id="reading-no-blanks_form-<?php echo $practise['id'];?>">
		<div class="table-container">
					...
					if(str_contains($question,'@@')) {
						if(substr_count($question,"@@")>2) {
							...
							echo $outValue = preg_replace_callback('/@@/', function ($m) use (&$key1, &$ans, &$data, &$question, &$k, &$pp) {
								...
								<span contenteditable="true" class="enter_disable spandata write-span fillblanks stringProperNew text-left">'.$newans.'</span>
			...
			<ul class="list-inline list-buttons" style="    margin-top: 20px;">
				<li class="list-inline-item"><button type="button" class="save_btn btn btn-primary btnSubmits_reding" data-pid="{{$practise['id']}}"
					data-toggle="modal" data-is_save="0" data-target="#exitmodal">Save</button>
				</li>
				<li class="list-inline-item"><button type="button" class="submit_btn btn btn-primary submitBtn btnSubmits_reding" data-pid="{{$practise['id']}}" data-is_save="1">Submit</button>
				</li>
			</ul>
		</div>
		</form> <?php 
			...
			<form class="reading-no-blanks_form commonFontSize" id="reading-no-blanks_form-<?php echo $practise['id'];?>">
				...
				if($practise['typeofdependingpractice'] == "set_full_view_remove_top_zero" && $practise['depending_practise_details']['question_type'] == "four_table_option"){
					...
					<script>
						...
						data7["{{$practise['id']}}"]= {"dependentpractice_ans": "{{$data[$practise['id']]['dependentpractice_ans']}}" };
						data7["{{$practise['id']}}"]["isRolePlayPractice"] = "{{$data[$practise['id']]['isRolePlayPractice']}}"
						data7["{{$practise['id']}}"]["is_dependent"] = "{{$data[$practise['id']]['is_dependent']}}"; 
						if(data7["{{$practise['id']}}"]["is_dependent"]==1){
							if(data7["{{$practise['id']}}"]["dependentpractice_ans"]==0 ){
					...
					<script>
						data7["{{$practise['id']}}"]["typeofdependingpractice"] = "{{ $data[$practise['id']]['typeofdependingpractice'] }}";
						data7["{{$practise['id']}}"]["dependant_practise_question_type"]  = "{{ $data[$practise['id']]['dependant_practise_question_type'] }}";
						if(data7["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_hide_show" || data7["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_parentextra" || data7["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_gen_que_double" || data7["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero_parentextra_get_questions_and_answers" || data7["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_zero" || data7["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_get_ans" || data7["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view" || data7["{{$practise['id']}}"]["typeofdependingpractice"] =="set_full_view_remove_top_zero"){

							data7["{{$practise['id']}}"]["dependant_practise_task_id"] = "{{ $data[$practise['id']]['dependant_practise_task_id'] }}";
							data7["{{$practise['id']}}"]["dependant_practise_id"] = "{{ $data[$practise['id']]['setFullViewFromPreviousPractice'] }}";
							$(function () {
							            // $('.cover-spin').fadeIn();
							          });
							if(data7["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){

										// IF DEPENDENT PRACTICE BELONGS TO SAME TASK THEN USE BELOW CODE.
										if(data7["{{$practise['id']}}"]["dependant_practise_id"] !=""  ){
											setTimeout(function(){ 
												data7["{{$practise['id']}}"]["prevHTML"] = $(document).find('.course-tab-content').find('#abc-'+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();				 
												$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).html(data7["{{$practise['id']}}"]["prevHTML"]);
												$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
												$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.removebutton').remove();
												$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.alert').remove();
												if( data7["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero"){
													$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.audio-player').remove();

													if(data7["{{$practise['id']}}"]["dependant_practise_question_type"] == "writing_at_end_speaking_up") {
														$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('fieldset').remove();
													}
													if(data7["{{$practise['id']}}"]["dependant_practise_question_type"] == "set_in_order_vertical_listening") {
														$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.audio-player').remove();
													}
												}
							                    // $('.cover-spin').fadeOut();
										...
										var baseUrl = "{{url('/')}}";
										data7["{{$practise['id']}}"]["dependant_practise_topic_id"] = "{{ $data[$practise['id']]['dependant_practise_topic_id'] }}";
										data7["{{$practise['id']}}"]["dependentURL"] = baseUrl+'/topic/'+data7["{{$practise['id']}}"]["dependant_practise_topic_id"]+'/'+data7["{{$practise['id']}}"]["dependant_practise_task_id"]+'?n='+data7["{{$practise['id']}}"]["dependant_practise_id"];
										$.get(data7["{{$practise['id']}}"]["dependentURL"],  //
										function (dataHTML, textStatus, jqXHR) {  // success callback
											data7["{{$practise['id']}}"]["prevHTML"] = $(dataHTML).find('.course-tab-content').find('#abc-'+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('form').html();
											$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).html(data7["{{$practise['id']}}"]["prevHTML"]);
											$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
											$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find(".blankTableBtn_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).remove();
											$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find(".topic_id").remove();
											$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find(".task_id").remove();
											$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find(".is_save").remove();
											$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find(".practise_id").remove();
											$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find(".alert-success").remove();
											$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find(".alert-danger").remove();
						...
						if($practise['typeofdependingpractice'] == "get_questions_and_answers" && $practise['depending_practise_details']['question_type'] == "reading_no_blanks_listening"){
							...
							<script>
								...
								if(data7["{{$practise['id']}}"]["is_dependent"]==1){
									...
									if(data7["{{$practise['id']}}"]["dependentpractice_ans"]==0 ){
										$(".previous_practset_full_viewice_answer_exists_{{$practise['id']}}").hide();
										$("#dependant_pr_{{$practise['id']}}").show();
									} else {
										$(".previous_practice_answer_exists_{{$practise['id']}}").show();
										$("#dependant_pr_{{$practise['id']}}").hide();
									...
									if(data7["{{$practise['id']}}"]["dependant_practise_task_id"] == "{{request()->segment(3)}}"){
							            ...
							            if(data7["{{$practise['id']}}"]["dependant_practise_id"] !=""  ){
							            	setTimeout(function(){ 
							            		...
							            		$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).css('pointer-events','none');
							            		$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.removebutton').remove();
							            		$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.list-buttons').remove();
							            		$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.alert').remove();
							            		if(data7["{{$practise['id']}}"]["typeofdependingpractice"]=="set_full_view_remove_zero"){
							            			$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.audio-player').remove();
							            			if(data7["{{$practise['id']}}"]["dependant_practise_question_type"] == "writing_at_end_speaking_up") {
							            		...
							            		$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.task_id').remove();
							            		$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('.practise_id').remove();
							            		$(document).find(".showPreviousPractice_reding_"+data7["{{$practise['id']}}"]["dependant_practise_id"]).find('input').each(function(){
					...
					// dd($exploded_question);
								...
								if(str_contains($question,'@@')) {
									...
									if(substr_count($question,"@@")>2){
										echo $outValue = preg_replace_callback('/@@/', function ($m) use (&$key1, &$ans, &$data, &$question, &$k, &$pp) {
											...
											$pp++;
											$str ='<span class="resizing-input1">
											<span contenteditable="true" class="enter_disable spandata fillblanks stringProperNew text-left" style="min-width:'.$style.' !important;">'.$newans.'</span>
											<input type="hidden" class="form-control form-control-inline appendspan" name="blanks[]" value="'.$newans.'">
											</span>';
											$k++;
											return $str;
									...
									<?php } 
									$questin = explode("<br><br>",$question);
									foreach ($questin as $key => $value) {
										?>
						...
						<ul class="list-inline list-buttons removebutton" >
							<li class="list-inline-item"><button type="button" class="save_btn btn btn-primary btnSubmits_reding" data-pid="{{$practise['id']}}" data-toggle="modal" data-is_save="0" data-target="#exitmodal">Save</button></li>
							<li class="list-inline-item">
								<button type="button" class="submit_btn btn btn-primary submitBtn btnSubmits_reding" data-pid="{{$practise['id']}}" data-is_save="1">Submit</button>
							</li>
						</ul>
												...
												var word = $.trim(words[i].replace(/^\s+/,""));
													...
													if(word !=""){
														...
														if(array.indexOf(wordNumber) =="-1"){
															...
															paragraph+="<span class='resizing-input1 mx-2'><span contenteditable='true' class='enter_disable spandata fillblanks stringProperNew text-left' style='min-width:"+size+"'>"+nans+"</span><input type='hidden' class='form-control form-control-inline appendspan' name='blanks[]' value="+nans+"></span>";
															temp++;
															if(i==115){
																paragraph+="<br><br>";
															}
						    				...
						    				$this.attr('data-total_words', str.split(' ').length);
													...
													for(var i=0; i<words.length;i++){
														...
														if(word !=""){
															...
															}<span class="resizing-input1" >
								<span contenteditable="true" class="enter_disable spandata fillblanks stringProperNew text-left">'.$newans.'</span>
								<input type="hidden" class="form-control form-control-inline appendspan" name="blanks[]" value="'.$newans.'">
															...
															if(array.indexOf(wordNumber) =="-1"){
																...
																$this.html(paragraph);
														  		...
														  		paragraph+="<span class='resizing-input1'><span contenteditable='true' class='enter_disable spandata fillblanks stringProperNew text-left' style='min-width:"+size+"'>"+nans+"</span><input type='hidden' class='form-control form-control-inline appendspan' name='blanks[]' value="+nans+"></span>";
														  		temp++;
														  		$this.html(paragraph);
